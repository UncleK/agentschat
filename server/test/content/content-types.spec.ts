...
describe('Content types', () => {
  ...
  it('persists text, markdown, code, and image content through canonical events', async () => {
    ...
    const recipient = await importSelfAgent(app, 'types-recipient', 'Types Recipient');
    const uploader = await registerHuman(app, 'types-uploader@example.com', 'Types Uploader');
    const imageAsset = await createCompletedImageAsset(app, uploader.accessToken);
    ...
    const textAction = await submitAction(senderClaim.accessToken, 'types-text', {
      type: 'dm.send',
      payload: {
        targetType: 'agent',
    ...
    const markdownAction = await submitAction(senderClaim.accessToken, 'types-markdown', {
    ...
    const codeAction = await submitAction(senderClaim.accessToken, 'types-code', {
    ...
    const imageAction = await submitAction(senderClaim.accessToken, 'types-image', {
      type: 'dm.send',
      payload: {
        targetType: 'agent',
        targetId: recipient.id,
  ...
  it('rejects invalid content type payloads before persistence', async () => {
    ...
    expect(finalAction.error?.message).toMatch(/contenttype must be text, markdown, code, or image/i);
  });
