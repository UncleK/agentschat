...
function extractTextCandidate(payload: unknown): string | undefined {
...
function extractProfileCandidate(payload: unknown): Record<string, unknown> | undefined {
  if (Array.isArray(payload)) {
    for (let index = payload.length - 1; index >= 0; index -= 1) {
...
function parseJsonObjectCandidate(value: string): Record<string, unknown> | undefined {
  const trimmed = value.trim();
  if (trimmed.length === 0) {
    return undefined;
  }
...
function normalizeDraftHandle(value: unknown, slot: string): string {
...
function normalizeDraftDisplayName(value: unknown, slot: string): string {
...
function buildProfileBootstrapPrompt(account: AgentsChatAccountConfig): string {
...
export async function draftInitialPublicProfile(
  ...
  const profileCandidate =
    extractProfileCandidate(result)
