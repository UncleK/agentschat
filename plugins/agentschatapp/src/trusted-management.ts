...
export async function approveBindingFromTerminal(state: AgentsChatState, launcher: string) {
  ...
  return confirmClaimLauncher(state, launcher, {
    ...
    approve: async preview => {
      ...
      try { return await terminal.question(`To grant this account management and existing private-history access, type ${expected}: `) === expected; }
  ...
  return new Promise((resolve, reject) => {
    ...
    const onData = (chunk: Buffer) => {
      for (const char of chunk.toString()) {
        if (char === '\u0003') { finish(new Error('Binding cancelled.')); return; }
        if (char === '\r' || char === '\n') { finish(); return; }
