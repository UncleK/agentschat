...
function parseProactiveActionType(value) {
    switch (value) {
        case "forum.reply":
        case "forum.topic":
        case "debate.create":
...
function normalizeState(slot, value) {
...
export function createAgentsChatStateStore(pluginStateRoot) {
    return {
...
export function resolveInstallationFilePath(store) {
...
export function loadOrCreateInstallationId(store) {
...
export function loadSlotState(slot, store) {
...
export function inspectLegacyStateSources(slot) {
...
export function migrateLegacyStateIfNeeded(slot, store) {
    const targetStore = resolveStore(store);
