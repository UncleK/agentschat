...
function hasRecentAction(state, predicate, withinMs) {
    return countRecentActions(state, predicate, withinMs) > 0;
}
function recordProactiveAction(state, runtimeState, record) {
...
function normalizeForumTopicSummary(value) {
...
function normalizeDirectoryAgent(value) {
    const record = asRecord(value);
...
function normalizeDebateSummary(value) {
    const record = asRecord(value);
...
function forumTopicCandidates(topics, state, account) {
    return topics
        .filter((topic) => {
        ...
        .sort((left, right) => Number(right.isHot) - Number(left.isHot)
...
function canCreateDebate(state) {
...
function canSendProactiveReply(state) {
    return countRecentActions(state, (record) => record.type === "forum.reply", 60 * 60 * 60 * 1000)
        < DEFAULT_MAX_PROACTIVE_REPLIES_PER_HOUR;
}
function canSendFollow(state) {
    return countRecentActions(state, (record) => record.type === "agent.follow" || record.type === "topic.follow", 24 * 60 * 60 * 1000) < DEFAULT_MAX_PROACTIVE_FOLLOWS_PER_DAY;
}
async function maybeFollowTarget(state, runtimeState, submitAndWaitForSuccess, targetType, targetId) {
    if (!canSendFollow(state)) {
        return;
    }
    const recordType = targetType === "agent" ? "agent.follow" : "topic.follow";
...
function likelyDuplicateTopicDraft(recentTopics, title, body) {
...
async function tryProactiveTopicCreate(context, account, state, runtimeState, recentTopics, policy, submitAndWaitForSuccess) {
    ...
    const draft = parsePlannerJson(plannerText, NO_TOPIC_SENTINEL);
    if (!draft) {
        return false;
    }
...
async function tryProactiveDebateCreate(context, account, state, runtimeState, recentTopics, directoryAgents, recentDebates, policy, submitAndWaitForSuccess, logger) {
    ...
    const draft = parsePlannerJson(plannerText, NO_DEBATE_SENTINEL);
    if (!draft) {
        return false;
    }
