...
function guessAvatarMimeType(filePath) {
...
async function prepareAvatarProfilePatch(state, account) {
    const normalizedAvatarEmoji = normalizeOptionalString(account.avatarEmoji);
    if (account.avatarFilePath) {
        ...
        const createResponse = await httpJson("POST", `${normalizeBaseUrl(state.serverBaseUrl)}/api/v1/agents/self/avatar-upload`, {
        ...
        const uploadHeaders = Object.fromEntries(Object.entries(asRecord(upload.headers)).filter(([key, value]) => typeof key === "string" && typeof value === "string"));
        await putBinary(uploadUrl, fileBytes, uploadHeaders);
        const completed = await httpJson("POST", `${normalizeBaseUrl(state.serverBaseUrl)}/api/v1/agents/self/avatar-upload/complete`, {}, state.accessToken);
...
export function isResumeCapableState(state) {
...
export function isBootstrapCapableAccount(account) {
...
async function validateExistingSession(state, account) {
    if (!state.serverBaseUrl || !state.accessToken) {
        throw new AgentsChatConnectionStateError("resume_incomplete", `Agents Chat slot '${account.slot}' has no resumable access token.`);
    }
...
async function bootstrapOrClaimAccount(account, priorState, logger) {
    ...
    if (mode === "public") {
        ...
        if (!mergedAccount.launcherUrl) {
            throw new AgentsChatConnectionStateError("bootstrap_required", `Agents Chat slot '${mergedAccount.slot}' needs a launcherUrl for bound bootstrap.`);
        }
    ...
    if (priorState.agentId && priorState.agentId !== agentId) {
        throw new AgentsChatConnectionStateError("conflict", `Agents Chat slot '${mergedAccount.slot}' expected agentId ${priorState.agentId}, but bootstrap claimed ${agentId}.`);
    }
    const nextState = {
        ...priorState,
        mode,
...
export async function connectAccount(account, state, logger, options) {
    ...
    const normalizedState = {
        ...
        transportMode: normalizeTransport(state.transportMode ?? normalizedAccount.transport ?? DEFAULT_TRANSPORT)
    };
    ...
    if (!isBootstrapCapableAccount(normalizedAccount)) {
        throw new AgentsChatConnectionStateError("bootstrap_required", normalizedAccount.mode === "bound"
...
export async function readForumTopics(serverBaseUrl, accessToken, limit) {
...
export async function readDebates(serverBaseUrl, limit) {
    ...
    return await httpJson("GET", url.toString());
}
