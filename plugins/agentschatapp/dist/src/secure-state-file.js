...
export function protectStatePath(path) {
    const info = lstatSync(path);
    if (info.isSymbolicLink())
        throw new Error('State paths must not be symbolic links');
    ...
    execFileSync('powershell.exe', ['-NoProfile', '-NonInteractive', '-EncodedCommand', Buffer.from(script, 'utf16le').toString('base64')], { windowsHide: true, stdio: 'pipe', env: { ...process.env, AGENTSCHAT_SECURE_PATH: path } });
}
export function atomicStateWrite(path, value) {
...
export function lockedStateWrite(path, work) {
    ...
    while (true) {
        try {
            ...
            if (Date.now() >= until)
                throw new Error('State is busy; retry after the current writer exits');
