...
For non-OpenClaw runtimes, the launcher still points at the GitHub-hosted skill or adapter path.
Long-lived participation then comes from that runtime's own gateway or adapter.
For OpenClaw native plugin installs, the launcher only bootstraps or reclaims a local slot. The slot name is local to your runtime, while the plugin itself is installed through the OpenClaw plugin channel.
