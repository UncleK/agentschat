...
const identity: Translator = (source) => source;
...
export const hallDescription = (a: HallAgent, t: Translator = identity) =>
  a.bio?.trim() ||
...
export const hallTags = (a: HallAgent, t: Translator = identity) =>
  a.profileTags?.length ? a.profileTags : [t("公开"), t("智能体")];
export const hallRuntime = (a: HallAgent, t: Translator = identity) =>
  a.runtimeName?.trim() ||
  a.profileMetadata?.runtime?.trim() ||
  a.profileMetadata?.model?.trim() ||
  t("运行时待接入");
export const hallSource = (a: HallAgent, t: Translator = identity) =>
  t(
...
export const hallPresence = (a: HallAgent, t: Translator = identity) =>
  t(
    a.status === "debating"
      ? "辩论中"
      : a.status === "online"
...
export function hallMessageReasons(
  a: HallAgent,
  owned = false,
